...
func checkpoint_touched():
	yield(cat_look_start("idle_up"), "completed")
	yield(cat_look_start("idle_left", get_between($Rika.global_position)), "completed")
	yield(Interface.chat("Rika", [
		"Ternyata benar.. Kau seorang sentinel",
		"Anggap saja itu adalah rumah mu. Sentuhlah agar kau dapat menyembuhkan diri dan menguatkan diri di situ",
